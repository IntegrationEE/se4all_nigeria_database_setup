...
WHERE state_name = 'Plateau';

-- insert data from import for survey_small_geneators Plateau 1
...
FROM import.offgrid_survey_small_generators_plateau_wo_1
WHERE generator != 'no generator';


-- insert data from import for survey_small_geneators Plateau 2
INSERT INTO se4all.cluster_offgrid_survey_small_generators (
    cluster_offgrid_id,
    wo_number,
    settlement_wo_id,
...
SELECT 
    ...
    CASE  -- business_description
        ...
        WHEN(fac_type = 'public_amenity') THEN (
            SELECT
            CASE
                WHEN (otherpubli = 'other public amenity') THEN other_publ
                ELSE otherpubli
            END
        )
    END,
    ...
    END,
    ST_Transform(geom,3857)
FROM import.offgrid_survey_small_generators_plateau_wo_2
WHERE generator != 'no generator';


-- database log (project,version,io,schema_name,table_name,script_name,comment)
SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_survey_small_generators','cluster_offgrid_survey_processing_small_generators_plateau.sql','Insert data');
