...
SELECT 
    ...
    TO_DATE(enteradate::VARCHAR, 'YYYY-MM-DD'), -- survey_date
    REPLACE(state_chec, '-', ' ' ), -- state_name
    INITCAP(lganame), -- lga_name
    ...
    CASE -- grid_functionality
        WHEN(fa = 'Functional' AND fb = 'Functional') THEN 'everything_functional'
        WHEN(fb = 'Functional') THEN 'transformer_only_functional'
        WHEN(fa = 'Never functional' OR fb = 'Never functional') THEN 'never_functional'
    END,
    ...
    aw::BOOLEAN, -- crop_cultivated_cashew
    ax::BOOLEAN, --crop_cultivated_yam
    ay::BOOLEAN, --crop_cultivated_cocoa
    ...
    CASE  -- banks_available
        WHEN(banks = '0') THEN FALSE
        ELSE TRUE
    END, 
    ...
    ce::INTEGER,-- count_traders_monday
    cf::INTEGER,-- count_traders_tuesday
    cg::INTEGER,-- count_traders_wednesday
    ch::INTEGER,-- count_traders_thursday
    ci::INTEGER,-- count_traders_friday
    cj::INTEGER,-- count_traders_saturday
    ck::INTEGER,-- count_traders_sunday
    ...
    household, -- households_with_genset_percentage 
    averagesiz::real, -- households_genset_size_avg_kva
    businesses, -- businesses_with_genset_percentage 
    cp::real, -- businesses_genset_size_avg_kva
    CASE -- network_mtn
    ...
    da::INTEGER, -- count_productive_use_other
    LOWER(db), -- name_productive_use_other
    ...
    teashopmai::INTEGER,  -- count_commercial_tea_shop
    posshop::INTEGER,  -- count_commercial_pos_shop
    dj::INTEGER, -- count_commercial_other
    LOWER(dk), -- name_commercial_other
    ...
    "dt"::INTEGER, -- count_craft_other
    LOWER(du), -- name_craft_other
...
FROM import.offgrid_survey_jigawa_wo_1;
...
SELECT 
    ...
    wo_check::INTEGER, -- wo_number
    settlement::INTEGER, -- settlement_wo_id
    TO_DATE(enteradate::VARCHAR, 'DD/MM/YYYY'), -- survey_date
    REPLACE(state_chec, '-', ' ' ), -- state_name
    INITCAP(lganame), -- lga_name
    INITCAP(communityn), -- community_name
    CASE  -- grid_presence
    ...
    --mg_gen_typ, -- mini_grid_primary_generation_type
    ST_Transform(geom,3857)
FROM import.offgrid_survey_jigawa_wo_2;
...
SELECT 
    ...
    --mg_gen_typ, -- mini_grid_primary_generation_type
    ST_Transform(geom,3857)
FROM import.offgrid_survey_jigawa_wo_3;
...
-- insert data from import for SURVEY Jigawa 4
INSERT INTO se4all.cluster_offgrid_survey (
    cluster_offgrid_id, 
    wo_number, 
    settlement_wo_id,
...
SELECT 
   ...
   --mg_gen_typ,-- mini_grid_primary_generation_type
    ST_Transform(geom,3857)
FROM import.offgrid_survey_jigawa_wo_4;
...
-- SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_survey','cluster_offgrid_survey_processing_jigawa.sql','Insert data');
