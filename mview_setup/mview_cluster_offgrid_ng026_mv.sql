/*
Create materialized view of cluster_offgrid for ng026
...
-- 1. Create temporary materialized view for the buffer with selected state and removing very small buildings
DROP MATERIALIZED VIEW IF EXISTS se4all.cluster_offgrid_ng026_buffer_mv CASCADE;
CREATE MATERIALIZED VIEW se4all.cluster_offgrid_ng026_buffer_mv AS
SELECT
    g.path[1] AS gid,
...
FROM
    ...
    WHERE s_jobdlabm.adm1_pcode = 'NG026' AND s_jobdlabm.area_m2 > 4)
AS g;

-- 2. Create temporary materialized view for the negative buffer
DROP MATERIALIZED VIEW IF EXISTS se4all.cluster_offgrid_ng026_negative_buffer_mv CASCADE;
CREATE MATERIALIZED VIEW se4all.cluster_offgrid_ng026_negative_buffer_mv AS
SELECT
    g.path[1] AS gid,
    g.geom::geometry(Polygon, 3857) AS geom
FROM
    ...
    FROM se4all.cluster_offgrid_ng026_buffer_mv AS s_cobm)
AS g;

-- 3. Count number of buildings within the cluster
DROP MATERIALIZED VIEW IF EXISTS se4all.cluster_offgrid_ng026_count_mv CASCADE;
CREATE MATERIALIZED VIEW se4all.cluster_offgrid_ng026_count_mv AS
SELECT
    s_conbm.gid, 
    COUNT(s_jobdlabm.geom) AS building_count,
    SUM(s_jobdlabm.area_m2)/(1000*1000) AS building_area_km2,
    s_conbm.geom AS geom
FROM se4all.cluster_offgrid_ng026_negative_buffer_mv AS s_conbm 
...
-- 4. Fill holes and remove clusters with a count of 6 buildings or less
DROP MATERIALIZED VIEW IF EXISTS se4all.cluster_offgrid_ng026_filter_mv CASCADE;
CREATE MATERIALIZED VIEW se4all.cluster_offgrid_ng026_filter_mv AS
SELECT
    s_concm.gid, 
    s_concm.building_count,
    s_concm.building_area_km2,
    ST_MakePolygon(ST_ExteriorRing(s_concm.geom)) AS geom
FROM se4all.cluster_offgrid_ng026_count_mv AS s_concm 
WHERE s_concm.building_count > 6;

-- 5. Create final materialized view
DROP MATERIALIZED VIEW IF EXISTS se4all.cluster_offgrid_ng026_mv CASCADE;
CREATE MATERIALIZED VIEW se4all.cluster_offgrid_ng026_mv AS
SELECT
    s_confm.gid, 
    ...
    s_confm.geom
FROM se4all.cluster_offgrid_ng026_filter_mv AS s_confm 
...
-- index GIST (geom)
CREATE INDEX cluster_offgrid_ng026_mv_geom_idx
    ON se4all.cluster_offgrid_ng026_mv USING GIST(geom);

-- database log (project,version,io,schema_name,table_name,script_name,comment)
SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_ng026_mv','mview_cluster_offgrid_ng026_mv.sql','Add materialised view');