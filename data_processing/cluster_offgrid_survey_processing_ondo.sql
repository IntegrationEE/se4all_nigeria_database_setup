...
-- insert data from import for SURVEY Ondo 1 (there is no WO 2 in Ondo)
...
SELECT 
    ...
    TO_DATE("enter a da"::varchar, 'YYYY-MM-DD'), -- survey_date
    INITCAP(state_chec), -- state_name
    CASE -- lga_name
        WHEN("select lga" = '1') THEN 'Akoko North-East'
        WHEN("select lga" = '2') THEN 'Akoko North-West'
        WHEN("select lga" = '3') THEN 'Akoko South-East'
        WHEN("select lga" = '4') THEN 'Akoko South-West'
        WHEN("select lga" = '5') THEN 'Akure North'
        WHEN("select lga" = '6') THEN 'Akure South'
        WHEN("select lga" = '7') THEN 'Ese Odo'
        WHEN("select lga" = '8') THEN 'Idanre'
        WHEN("select lga" = '9') THEN 'Owo'
        WHEN("select lga" = '10') THEN 'Ifedore'
        WHEN("select lga" = '11') THEN 'Ondo East'
        WHEN("select lga" = '12') THEN 'Ondo West'
        WHEN("select lga" = '13') THEN 'Ilaje'
        WHEN("select lga" = '14') THEN 'Ile Oluji/Okeigbo'
        WHEN("select lga" = '15') THEN 'Irele'
        WHEN("select lga" = '16') THEN 'Odigbo'
        WHEN("select lga" = '17') THEN 'Okitipupa'
        WHEN("select lga" = '18') THEN 'Ose'
    END,
    ...
    END,
    "is there g"::int::BOOLEAN, -- grid_structure_presence
    "what year", -- grid_year_built
    poles::int::BOOLEAN, -- grid_poles_presence
    cable::int::BOOLEAN, -- grid_cable_presence
    transforme::int::BOOLEAN, -- grid_transformer_presence
    ...
    "main cro_1"::int::BOOLEAN, -- crop_cultivated_millet
    "main cro_2"::int::BOOLEAN, -- crop_cultivated_guinea_corn
    "main cro_3"::int::BOOLEAN, -- crop_cultivated_groundnut
    "main cro_4"::int::BOOLEAN, -- crop_cultivated_maize
    "main cro_5"::int::BOOLEAN, -- crop_cultivated_rice
    "main cro_6"::int::BOOLEAN, -- crop_cultivated_beans
    "main cro_7"::int::BOOLEAN, -- crop_cultivated_pepper
    "main cro_8"::int::BOOLEAN, -- crop_cultivated_sweet_potatoes
    "main cro_9"::int::BOOLEAN, -- crop_cultivated_hibiscus_leaf
    "main cro10"::int::BOOLEAN, -- crop_cultivated_sesame_seed
    "main cro11"::int::BOOLEAN, -- crop_cultivated_cassava
    ...
    "mobile mon"::int::BOOLEAN,-- mobile_money_available
    banks::int::BOOLEAN,-- banks_available
    monday::int::BOOLEAN,-- market_monday
    tuesday::int::BOOLEAN,-- market_tuesday
    wednesday::int::BOOLEAN,-- market_wednesday
    thursday::int::BOOLEAN,-- market_thursday
    friday::int::BOOLEAN,-- market_friday
    saturday::int::BOOLEAN,-- market_saturday
    sunday::int::BOOLEAN,-- market_sunday
    ...
    END,   
    "petrol sta"::int::BOOLEAN, -- petrol_station_in_community
    ...
    mg_present::int::BOOLEAN, -- mini_grid_presence
    mg_status, -- mini_grid_status
    mg_gen_typ, -- mini_grid_primary_generation_type
    ST_Transform(geom,3857)
FROM import.offgrid_survey_ondo_wo_1;
...
SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_survey','cluster_offgrid_survey_processing_ondo.sql','Insert data');