...
WHERE state_name = 'Cross River';

-- had to remove some text in settlement ID 8 in import schema to fit 50 char: Bar-1, Restaurant-3, Fuel shop-10, viewing centre-1, spare parts shop-4 changed to Bar-1, Restaurant-3, Fuel shop-10
-- insert data from import for SURVEY Cross River 1
...
FROM import.offgrid_survey_cross_river_wo_1;


-- insert data from import for SURVEY Cross River 2
INSERT INTO se4all.cluster_offgrid_survey (
    cluster_offgrid_id, 
    wo_number, 
    settlement_wo_id,
...
SELECT 
   ...
   -- mg_status, -- mini_grid_status
    --mg_gen_typ, -- mini_grid_primary_generation_type
    ST_Transform(geom,3857)
FROM import.offgrid_survey_cross_river_wo_2;
...
-- SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_survey','cluster_offgrid_survey_processing_cross_river.sql','Insert data');
