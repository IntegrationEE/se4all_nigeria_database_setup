...
Post processing for cluster file
...
-- metadata
COMMENT ON TABLE import.cluster_all_import IS '
{"name": "cluster_all_import",
...
-- format verification
SELECT obj_description('import.cluster_all_import' ::regclass) ::json;

-- index GIST (geom)
CREATE INDEX cluster_all_import_geom_idx
    ON import.cluster_all_import USING GIST (geom);

-- grant (grp_admin)
ALTER TABLE import.cluster_all_import OWNER TO grp_admin;

-- database log (project,version,io,schema_name,table_name,script_name,comment)
SELECT database_log('se4all','v0','setup','import','cluster_all_import','cluster_all_import_postprocessing.sql','Comment metadata');
