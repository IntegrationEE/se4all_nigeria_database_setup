...
WHERE state_name = 'Imo';

--had to remove some text in settlement ID 12 in import schema to fit 50 char: Pool Billiard, Restauarant, Alcohol shop, Agragrian shop
--had to remove some text in settlement ID 9 in import schema to fit 50 char: Entertainment (dance group.)Abigbo LORJI dance group.Women dance group.Ezialla, Umugama, Eziamanta.
--had to remove some text in settlement ID 22 in import schema to fit 50 char: Bar shop, Fuel shop, Motorcycle parts shop, Agrarian shop
...
FROM import.offgrid_survey_imo_wo_1;
...
--SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_survey','cluster_offgrid_survey_processing_imo.sql','Insert data');
