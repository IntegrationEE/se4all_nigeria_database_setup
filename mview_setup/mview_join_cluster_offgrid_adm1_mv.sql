...
DROP MATERIALIZED VIEW IF EXISTS se4all.join_cluster_offgrid_adm1_mv CASCADE;
CREATE MATERIALIZED VIEW se4all.join_cluster_offgrid_adm1_mv(
    cluster_offgrid_id                  ,
    adm1_pcode                      ) AS
SELECT 
    s_co.cluster_offgrid_id,
    s_bab.adm1_pcode
FROM se4all.cluster_offgrid AS s_co
LEFT JOIN se4all.boundary_adm1_buffer_mv AS s_bab
    ON ST_Intersects(s_co.geom, s_bab.geom)
	WHERE s_co.geom && s_bab.geom;

-- database log (project,version,io,schema_name,table_name,script_name,comment)
SELECT database_log('se4all','v0','setup','se4all','join_cluster_offgrid_adm1_mv','mview_join_cluster_offgrid_adm1_mv.sql','Add materialised view');
