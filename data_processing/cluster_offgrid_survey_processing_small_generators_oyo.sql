...
-- insert data from import for survey_small_geneators OYO 1
...
FROM import.offgrid_survey_small_generators_oyo_wo_1
WHERE generator != 'no generator';


-- insert data from import for survey_small_geneators OYO 2
INSERT INTO se4all.cluster_offgrid_survey_small_generators (
    cluster_offgrid_id,
    wo_number,
    settlement_wo_id,
...
SELECT 
    ...
    END,
    ST_Transform(geom,3857)
FROM import.offgrid_survey_small_generators_oyo_wo_2
WHERE generator != 'no generator';


-- insert data from import for survey_small_geneators OYO 3
INSERT INTO se4all.cluster_offgrid_survey_small_generators (
    cluster_offgrid_id,
    wo_number,
    settlement_wo_id,
...
SELECT 
    ...
    END,
    ST_Transform(geom,3857)
FROM import.offgrid_survey_small_generators_oyo_wo_3
WHERE generator != 'no generator';

-- database log (project,version,io,schema_name,table_name,script_name,comment)
SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_survey_small_generators','cluster_offgrid_survey_processing_small_generators_oyo.sql','Insert data');
