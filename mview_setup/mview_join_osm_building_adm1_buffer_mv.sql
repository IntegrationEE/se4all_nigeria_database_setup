...
DROP MATERIALIZED VIEW IF EXISTS se4all.join_osm_building_adm1_buffer_mv CASCADE;
CREATE MATERIALIZED VIEW se4all.join_osm_building_adm1_buffer_mv(
    osm_building_id,
    area_m2,
    adm1_pcode,
    geom                      ) AS
SELECT 
    s_ob.osm_building_id,
    s_ob.area_m2,
    s_babm.adm1_pcode,
    s_ob.geom
FROM se4all.osm_building AS s_ob
LEFT JOIN se4all.boundary_adm1_buffer_mv AS s_babm
    ON ST_Intersects(s_ob.geom, s_babm.geom)
	WHERE s_ob.geom && s_babm.geom;

-- index GIST (geom)
CREATE INDEX join_osm_building_adm1_buffer_mv_geom_idx
    ON se4all.join_osm_building_adm1_buffer_mv USING GIST(geom);

-- database log (project,version,io,schema_name,table_name,script_name,comment)
SELECT database_log('se4all','v0','setup','se4all','join_osm_building_adm1_buffer_mv','mview_join_osm_building_adm1_buffer_mv.sql','Add materialised view');
