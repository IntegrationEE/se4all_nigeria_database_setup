...
WHERE state_name = 'Delta';

-- insert data from import for survey_small_geneators Delta 1
...
FROM import.offgrid_survey_small_generators_delta_wo_1
WHERE generator != 'no generator';

-- database log (project,version,io,schema_name,table_name,script_name,comment)
SELECT database_log('se4all','v0','setup','se4all','cluster_offgrid_survey_small_generators','cluster_offgrid_survey_processing_small_generators_delta.sql','Insert data');
